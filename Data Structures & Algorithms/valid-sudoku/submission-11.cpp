...
// Using Bitmask
            ...
            for (int r = 0; r < 9; r++) {
                for (int c = 0; c < 9; c++) {
                    ...
                    if ((cols[c] & (1 << board[r][c] - 1)) || ((rows[r] & (1 << board[r][c] - 1))) 
                        || (sub_boxes[((r/3)*3 + c/3)] & (1 << board[r][c] - 1))) {return false;}
                    
                    cols[c] |= (1 << board[r][c] - 1);
                    rows[r] |= (1 << board[r][c] - 1);
                    sub_boxes[(r/3)*3 + (c/3)] |= (1 << board[r][c] - 1);
